...
local function get_workspace_information()
	...
	logger.debug(
		"Sending request to LSP Server",
		{ feature = "get-workspace-information", request = request, method = request_method }
	)
	...
	if response.err ~= nil then
		logger.error(
			"LSP client responded with error!",
			{ feature = "get-workspace-information", error = response.err, request = request, method = request_method }
		)
		return
	end
...
function M.select_project()
	...
	elseif #executable_projects == 1 then
		...
		logger.debug(
			"Found only one executable project",
			{ feature = "select-project", project = selected_project, workspace_information = workspace_information }
		)
		notify.info(string.format("Found only one executable project %s, using it.", selected_project.AssemblyName))
		return selected_project
	else
		logger.debug("Found multiple projects! Selecting one", {
			feature = "select-project",
			executable_projects = executable_projects,
